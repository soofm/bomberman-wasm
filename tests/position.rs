...
#[cfg(test)]
mod test {
    use bomberman_wasm::geometry::Position;
    ...
    fn object_can_move_over_empty_square_boundaries() {
        ...
        let mut obj = GameObject::new(0.8, 0.0);
        obj.move_to(1.2, 0.0, &tiles);
        assert_eq!(obj.position(), (1.2, 0.0));
    }
    ...
    #[test]
    fn object_cannot_move_diagonally_from_origin_to_empty_square() {
        let tiles = vec![Tile::Empty, Tile::HardBlock, Tile::HardBlock, Tile::Empty];
        let tiles = Tiles::new(tiles, 2, 2);                                                                               
        let mut obj = GameObject::new(0.0, 0.0);
        obj.move_to(1.0, 1.0, &tiles);
        assert_eq!(obj.position(), (0.0, 0.0));
    }
}
